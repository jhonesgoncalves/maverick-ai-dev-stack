...
async function buildProjectIndex(cwd) {
  ...
  await walk(root2, async (path) => {
    ...
    if (/\.(md|mdx|txt)$/i.test(path)) {
      const item = { path: rel2, title: title(body, rel2), excerpt: body.replace(/^---[\s\S]*?---/, "").replace(/\s+/g, " ").slice(0, 220), content: body, type: /SPEC\.md$/i.test(path) ? "SPEC" : /(?:^|\/)ADR[-_]?\d|architecture.*decision/i.test(rel2) ? "ADR" : /TASK\.md$/i.test(path) ? "TASK" : "DOC" };
      docs.push(item);
    ...
    } else if (/\.(js|jsx|ts|tsx)$/i.test(path)) {
      const imports = [...body.matchAll(/(?:from\s*|import\s*\(|require\()\s*['\"]([^'\"]+)['\"]/g)].map((match) => match[1]);
      modules.push({ path: rel2, kind: /test|spec/i.test(rel2) ? "test" : "module", confidence: "Detected", imports });
    }
  });
...
function json(res, status2, body) {
  res.writeHead(status2, { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" });
...
async function startDashboard({ cwd, host = "127.0.0.1", port = 4173, watch = true } = {}) {
  ...
  const server = createServer(async (req, res) => {
    if (req.url === "/api/project") return json(res, 200, index);
    if (req.url === "/api/refresh" && req.method === "POST") {
      await refresh();
      res.writeHead(204);
      return res.end();
    }
    if (req.url === "/api/improvements" && req.method === "POST") {
      const body = await requestBody(req), slug = String(body.slug || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 81);
      if (!/^[a-z0-9][a-z0-9-]{1,80}$/.test(slug)) return json(res, 400, { error: "A valid improvement title is required." });
      try {
        const { main: main2 } = await import("./cli-UAVM4FAV.js");
        await main2(["task", slug, "--preset", body.preset === "strict-review" ? "strict-review" : "spec-driven"], cwd);
        await refresh();
        return json(res, 201, { slug, preset: body.preset === "strict-review" ? "strict-review" : "spec-driven" });
      } catch (error) {
...
async function loadConfig(cwd) {
  ...
  if (await exists2(canonical)) return mergeConfig(await json2(canonical));
  if (await exists2(legacy)) return mergeConfig(await json2(legacy));
  return mergeConfig();
}
...
function parseOptions(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i].startsWith("--")) {
      ...
      options[key] = args[i + 1]?.startsWith("--") || args[i + 1] === void 0 ? true : args[++i];
...
async function taskPreset(dir, config) {
  ...
  if (await exists2(meta)) return (await json2(meta)).preset || config.workflow.defaultPreset;
...
async function main(args, cwd = process.cwd()) {
  const { positional, options } = parseOptions(args), [command, arg, third] = positional;
