export const PRESET_IDS = ['lightweight', 'standard', 'spec-driven', 'strict-review'];

const artifact = (id, file, template, sections) => ({ id, file, template, sections });
const common = {
...
export function getPreset(id = 'standard') { return presets[id]; }
export function requirePreset(id) { const preset = getPreset(id); if (!preset) throw new Error(`invalid preset "${id}"; choose: ${PRESET_IDS.join(', ')}`); return preset; }
export function stagesForPreset(id) { return requirePreset(id).stages; }
